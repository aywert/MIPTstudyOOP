...
std::list<Snake>::iterator Model::findSnakeById(int id) {
  return std::find_if(snakes_.begin(), snakes_.end(), 
    [id](const Snake& s){ return s.getID() == id; });
}

std::list<Snake>::iterator Model::removeSnake(int id) {
...
void Model::addRabbit(Rabbit rabbit) {
  rabbits_.push_back(rabbit);
}


void Model::spawnRabbit() {
...
bool Model::checkBoundaryCollision(const Snake& snake) {
...
bool Model::checkSnakeCollisions(const Snake& headSnake) {
...
void Model::handleRabbitCollision(Snake& snake) {
  const auto& head = snake.getHead();
...
void Model::handlePortalCollisions(Snake& snake) {
...
void Model::updateSnakeDirection(size_t playerIdx, Direction newDir) {
...
Direction Model::easy_bot_calcul_direction(const Snake& bot) {
...
Direction Model::smart_bot_calcul_direction(const Snake& bot) {
...
Rabbit Model::nearestRabbit(const Snake& snake) {
  const auto& head = snake.getHead();
...
bool Model::isPositionSafe(int x, int y) const {
  int min_x = shift_col + 1;
  int max_x = window_width_ - shift_col;
  int min_y = shift_row + 1;
  int max_y = window_height_ - shift_row;
...
std::vector<std::vector<double>> Model::generateHeatmap(int width, int height, const Snake& snake) {
...
Direction Model::chooseBestMoveBasedOnHeatMap(const Snake& snake, 
                int width, int height, const std::vector<std::vector<double>>& heatmap) {
  struct MoveOption {
    Direction dir;
    int dx;
    int dy;
